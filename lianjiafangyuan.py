...
def house_percell_spider(conn,cellname = u'荣丰2008'):
    ...
    url=u"http://bj.lianjia.com/ershoufang/rs"+cellname+"/"
...
#==============================================================================
    ...
    for page in range(total_pages):
        if page>0:
            url_page=u"http://bj.lianjia.com/ershoufang/pg%drs%s/" % (page+1,cellname)
            req = urllib2.Request(url_page,headers=hds[random.randint(0,len(hds)-1)])
            source_code = urllib2.urlopen(req,timeout=50).read()
...
def daily_report(conn):
    url = u"http://bj.lianjia.com/fangjia/"
    ...
    report = soup.find('div', {'class': 'box-l'})
    if report == None:
        exit(-1)

    today = get_today()
    info_dict.update({u'adate': today})
    chengjiao = report.find('div', {'class': 'qushi-2'})
    chengjiaonum = chengjiao.find('span', {'class': 'num'})
    # print u'成交 ' + chengjiaonum.get_text().strip()
    info_dict.update({u'chengjiaonum': chengjiaonum.get_text().strip()})

    nums = report.findAll('div', {'class': 'item item-1-3'})
    count = 1;
    for num in nums:
        # print num.get_text().strip()
        numCount = num.find('div', {'class': 'num'})
        key = u"num%d" % count;
        info_dict.update({key: numCount.get_text().strip()})
        count = count + 1
    ...
    t = tuple(t)
    cursor = conn.cursor()
    command = "insert into report (createdate,chengjiao,fangyuan,keyuan,daikan) values ('%s',%s,%s,%s,%s)" % t
