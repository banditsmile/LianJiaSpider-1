...
def get_proxyList():
    ...
    while len(proxyList)==0:
        url = 'http://dev.kuaidaili.com/api/getproxy/?orderid=928618036841666&num=3&b_pcchrome=1&b_pcie=1&b_pcff=1&protocol=1&method=1&an_tr=1&an_an=1&an_ha=1&sp1=1&sp2=1&quality=1&sort=1&dedup=1&format=json&sep=1'
        try:
            jsonData = get_json_data(url)
            value = json.loads(jsonData)
            session = requests.session()
            print value['data']
            proxy_list = value['data']['proxy_list']
            for proxyip in proxy_list:
                try:
                    testurl = 'http://ip.chinaz.com/getip.aspx'
                    proxy = {'http': proxyip}
                    print proxy
                    response = session.get(testurl, proxies=proxy, timeout=10)
...
def gen_chengjiao_insert_command(info_dict, conn):
    ...
    生成成交记录数据库插入命令
    """
    info_list = [u'编号', u'链接', u'房子名称', u'签约时间', u'签约单价', u'签约总价', u'基本信息1', u'基本信息2', u'基本信息3', u'小区编号', u'regionb',
                 u'regions']
    t = []
    for il in info_list:
        if il in info_dict:
...
def chengjiao_spider(db_cj, url_page=u"http://bj.lianjia.com/chengjiao/pg1rs%E5%86%A0%E5%BA%AD%E5%9B%AD",
                     xiaoqubianhao=u'1110138869312539', regionb=u'朝阳', regions=u'朝阳'):
    """
    爬取页面链接中的成交记录
    """
    try:
...
def do_xiaoqu_chengjiao_again_spider(db_xq, db_cj):
    ...
    count = 0
    cursor = db_xq.cursor()
    cursor.execute('select * from xiaoqu where baseinfo ="" ORDER BY bianhao DESC')
    xq_list = cursor.fetchall()
    for xq in xq_list:
        ...
        while result==False :
            print 'proxy is down: %s' % (proxyList[proxyIndex],)
            set_next_proxy()
            result = chengjiao_spider(db_cj, url, xq[0], xq[2], xq[3])
        if result:
            count += 1
            print 'have spidered %d xiaoqu, the latest is: %s %s %s %s' % (count, xq[0], xq[1], xq[4], proxyList[proxyIndex])
...
def database_init(dbflag='local'):
    ...
    dbc.execute(
        'create table if not exists hisprice (id int(10) NOT NULL AUTO_INCREMENT primary key,houseID varchar(50) , date varchar(50), totalPrice varchar(200))')
    dbc.execute(
        'create table if not exists cellinfo (id int(10) NOT NULL AUTO_INCREMENT primary key,Title varchar(50) , link varchar(200),district varchar(50),bizcircle varchar(50),tagList varchar(200))')

    dbc.execute(
        "create table if not exists xiaoqu (name varchar(200) primary key UNIQUE, regionb varchar(200), regions varchar(200), style varchar(200), year varchar(200))")

    dbc.execute(
        "create table if not exists chengjiao (href varchar(200) primary key UNIQUE, name varchar(200), style varchar(200), area varchar(200), orientation varchar(200), floor varchar(200), year varchar(200), sign_time varchar(200), unit_price varchar(200), total_price varchar(200),fangchan_class varchar(200), school varchar(200), subway varchar(200))")
